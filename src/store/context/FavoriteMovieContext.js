...
import {
  ...
  setIsLoading,
  setMaximumAmountOfFavoriteMovies,
  setIsError,
  setErrorMessage,
...
const initialState = {
  favoriteMovies: [],
  maximumAmountOfFavoriteMovies: 0,
  isLoading: false,
  isError: false,
  errorMessage: "",
}
...
export const FavoriteMovieProvider = ({ children }) => {
  ...
  useEffect(() => {
    const getFavoriteMovies = async () => {
      try {
        ...
        dispatch(setMaximumAmountOfFavoriteMovies(data.data.length))
  ...
  const addFavoriteMovie = async (movieId) => {
    ...
    try {
      ...
      dispatch(
        saveFavoritesToStore([...state.favoriteMovies, newFavoriteMovie.data])
      )
    } catch (error) {
      dispatch(setErrorMessage(error.message))
      dispatch(setIsError(true))
